...
exports.forgotPassword = async (req, res) => {
  ...
  // Validate email format with stricter regex (e.g., requiring a known TLD)
  //   const emailRegex = /^[^\s@]+@[^\s@]+\.(com|org|net|edu|gov|in|co\.in|co)$/i; // Case-insensitive, allows common TLDs
...
exports.verifyOtp = async (req, res) => {
  ...
  // Validate email format with stricter regex (e.g., requiring a known TLD)
  //   const emailRegex = /^[^\s@gmail]+@gmail[^\s@gmail]+\.(com|org|net|edu|gov|in)$/i; // Case-insensitive, allows common TLDs
  // *Or
  ...
  // check opt must 6 digits number
  if (!/^\d{6}$/.test(otp)) {
    return res.status(400).json({
